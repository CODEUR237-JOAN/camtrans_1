...
class EtatDemandeExpedition {
    ...
    switch (etape) {
      case 1:
        ...
        if (categorieService == "Remorque") {
          return marqueVehiculeRemorque.isNotEmpty &&
              modeleVehiculeRemorque.isNotEmpty;
        }
  ...
  EtatDemandeExpedition copierAvec({
    ...
    return EtatDemandeExpedition(
      ...
      marqueVehiculeRemorque:
          marqueVehiculeRemorque ?? this.marqueVehiculeRemorque,
      modeleVehiculeRemorque:
          modeleVehiculeRemorque ?? this.modeleVehiculeRemorque,
...
class DemandeExpeditionNotifier extends StateNotifier<EtatDemandeExpedition> {
  ...
  DemandeExpeditionNotifier(
      this.serviceIA, this.serviceFirestore, this.serviceGps)
      : super(EtatDemandeExpedition());
  ...
  void setLatitudeDepart(double val) =>
      state = state.copierAvec(latitudeDepart: val);
  void setLongitudeDepart(double val) =>
      state = state.copierAvec(longitudeDepart: val);
  void setLatitudeArrivee(double val) =>
      state = state.copierAvec(latitudeArrivee: val);
  void setLongitudeArrivee(double val) =>
      state = state.copierAvec(longitudeArrivee: val);

  void setTypeMarchandise(String val) =>
      state = state.copierAvec(typeMarchandise: val);
  void setCategorieVehicule(String val) =>
      state = state.copierAvec(categorieVehicule: val);
  void setDescription(String val) => state = state.copierAvec(description: val);
  void setDateTransport(DateTime val) =>
      state = state.copierAvec(dateTransport: val);
  void setHeureTransport(TimeOfDay val) =>
      state = state.copierAvec(heureTransport: val);

  // Sprint 10
  void setCategorieService(String val) =>
      state = state.copierAvec(categorieService: val);
  void setOptionGamme(String val) => state = state.copierAvec(optionGamme: val);
  void setDetailsSpecifiques(String val) =>
      state = state.copierAvec(detailsSpecifiques: val);

  // Remorque — setters spécifiques
  /// Alias : setMarque (correspond à setMarqueRemorque)
  void setMarque(String val) =>
      state = state.copierAvec(marqueVehiculeRemorque: val);

  /// Alias : setModele (correspond à setModeleRemorque)
  void setModele(String val) =>
      state = state.copierAvec(modeleVehiculeRemorque: val);
  void setMarqueRemorque(String val) =>
      state = state.copierAvec(marqueVehiculeRemorque: val);
  void setModeleRemorque(String val) =>
      state = state.copierAvec(modeleVehiculeRemorque: val);
  void setMasseEstimeeKg(double val) =>
      state = state.copierAvec(masseEstimeeKg: val);
  void setEstEnAttenteMasseIA(bool val) =>
      state = state.copierAvec(estEnAttenteMasseIA: val);
  ...
  Future<void> estimerMasseIA() async {
    ...
    try {
      final masse = await serviceIA.estimerMasseVehicule(
          state.marqueVehiculeRemorque, state.modeleVehiculeRemorque);
      state =
  ...
  Future<void> estimerAvecIA() async {
    ...
    try {
      estimation = await serviceIA.estimerExpedition(
        marchandise: state.typeMarchandise.isNotEmpty
            ? state.typeMarchandise
            : state.categorieService,
        description:
      ...
      estimation = {
        ...
        "prix": "Sur devis",
        "conseil":
            "Le système d'analyse est temporairement saturé. Un conseiller vérifiera vos détails.",
      };
    }
    ...
    state =
        state.copierAvec(latitudeDepart: latClient, longitudeDepart: lngClient);
