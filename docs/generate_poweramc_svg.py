...
def create_svg():
    ...
    <text x="960" y="52" class="text" text-anchor="middle" font-size="11" font-style="italic">COMMUNICATION TRAITER COURSE</text>
    '''
    ...
    <line x1="{ax}" y1="{ay+20}" x2="{ax+10}" y2="{ay+40}" class="line"/>
    <text x="{ax}" y="{ay+60}" class="text" text-anchor="middle">Transporteur</text>
    '''
    ...
    # Self System Line
    svg += f'<polyline points="{sx},{sy-sh/2} {sx},{sy-sh/2-30} {sx+sw/2+20},{sy-sh/2-30} {sx+sw/2+20},{sy-sh/2+10} {sx+sw/2},{sy-sh/2+10}" class="line"/>'
    ...
    # Transporteur <-> Systeme (Vertical Line)
    svg += draw_msg(ax - 80, 280, "1: affiche notification", 'up')
    svg += draw_msg(ax + 90, 320, "2: consulte détails course", 'down')
    svg += draw_msg(ax - 70, 360, "8: redirige vers GPS", 'up')
    svg += draw_msg(ax + 90, 420, "4: clique « Accepter »", 'down')

    # Transporteur <-> Systeme (Horizontal Line)
    svg += draw_msg(300, sy - 20, "4.a: [Refus] clique « Refuser »", 'right')
    svg += draw_msg(380, sy + 30, "5.a: [Annulée] affiche erreur", 'left')

    # Systeme <-> SGBD (Horizontal Line)
    svg += draw_msg(700, sy - 20, "5: vérifie disponibilité course", 'right')
    svg += draw_msg(750, sy + 30, "5.1: [Disponible] retourne statut", 'left')

    # Systeme <-> SGBD (Vertical Line)
    svg += draw_msg(bx + 80, 320, "6: lie transporteur &amp; MAJ", 'up')
    svg += draw_msg(bx - 90, 380, "5.b: [Annulée] retourne erreur", 'down')
    svg += draw_msg(bx + 80, 420, "7: MAJ statut Indisponible", 'up')

    # Self System Line
    svg += draw_msg(sx + 60, sy - 70, "4.b: [Timeout 30s] relance algo", 'left')


    svg += '</svg>'
    
    out_path = os.path.join('c:\\\\tmp\\\\update_camtrans\\\\docs', 'diagramme_poweramc_exact.svg')
